...
class AutoBioinformaticsUI:
    ...
    def __init__(self, root):
        ...
        Label(mainframe, text="Please enter the unique group identifiers separated by commas (e.g.: Group 1,Group 2,Group 3)").grid(row=3, column=1, columnspan=3, sticky=W)
        ...
        filepath_entry = Button(mainframe, text="Choose File", command=self.ask_for_filepath).grid(column=3, row=2, sticky=E)


        group_names_entry = Entry(mainframe, width=40, textvariable=self.group_names).grid(column=2, row=4, sticky=(W, E))

        gene_col_entry = Entry(mainframe, width=40, textvariable=self.gene_col).grid(column=2, row=5, sticky=(W, E))

        self.output_path_display = Label(mainframe, text="")
        self.output_path_display.grid(column=2, row=6, sticky=(W, E))
        filepath_entry = Button(mainframe, text="Choose Output Folder", command=self.ask_for_output_path).grid(column=3, row=6, sticky=E)

        generate_report_checkbox = Checkbutton(mainframe, variable=self.generate_report).grid(column=2, row=7, sticky=(W, E))

        Button(mainframe, text="Run Analysis", command=self.run_analysis).grid(column=3, row=7, sticky=E)
    ...
    def ask_for_filepath(self, *args):
        ...
        self.filepath_display.config(text= self.filepath[:10] + "..." + self.filepath[-10:])

    def ask_for_output_path(self, *args):
        """Ask the user for the output directory"""

        self.output_path = filedialog.askdirectory()
        self.output_path_display.config(text=self.output_path[:10] + "..." + self.output_path[-10:])
